  ...
  return (
    <div>
      <div className="border-b bg-white px-6 py-4 flex items-center gap-3">
        ...
        <h1 className="text-base font-semibold text-gray-900">Push Shared Goal</h1>
      </div>

      <div className="max-w-3xl mx-auto p-8">
        <div className="bg-orange-50 border border-orange-100 rounded-lg px-5 py-3 text-sm text-gray-700 mb-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" value={form.target} onChange={e => setForm({ ...form, target: e.target.value })}
                  className="w-full border border-gray-200 rounded px-3 py-2 text-sm" placeholder="e.g. 0" required />
              </div>
            </div>
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                ...
                {users.map(u => (
                  <div key={u.id} onClick={() => toggleEmployee(u.id)}
                    className={'flex items-center gap-3 px-4 py-3 cursor-pointer border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors ' + (selected.includes(u.id) ? 'bg-orange-50' : '')}>
                    <input type="checkbox" checked={selected.includes(u.id)} onChange={() => toggleEmployee(u.id)} />
                    <div>
                      <div className="text-sm font-medium text-gray-900">{u.name}</div>
                      <div className="text-xs text-gray-400">{u.department} &middot; {u.email}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {success && <p className="text-green-600 text-sm bg-green-50 p-3 rounded-lg border border-green-100">Shared goal pushed successfully!</p>}
